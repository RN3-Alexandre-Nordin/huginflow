...
const TABS: Array<{
  ...
  /** Se true, só renderiza com canAprovarRequisicoes */
  ...
  },
  {
    ...
    icon: ClipboardList,
    match: (p) =>
      p.startsWith('/cockpit/estoque/requisicoes') &&
      !p.startsWith('/cockpit/estoque/requisicoes/aprovacao'),
  },
  {
    href: '/cockpit/estoque/requisicoes/aprovacao',
    label: 'Aprovação',
    icon: ShieldCheck,
    match: (p) => p.startsWith('/cockpit/estoque/requisicoes/aprovacao'),
    activeIconClass: 'text-emerald-400',
    requerAprovacao: true,
  },
]

export default function EstoqueAreaNavClient({
  canAprovarRequisicoes = false,
}: {
  canAprovarRequisicoes?: boolean
}) {
  const pathname = usePathname()
  const tabs = TABS.filter((t) => !t.requerAprovacao || canAprovarRequisicoes)
