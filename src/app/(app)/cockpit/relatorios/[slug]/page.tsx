...
import BiReportFiltersForm from '@/components/relatorios/BiReportFiltersForm'
import BiReportExportBar from '@/components/relatorios/BiReportExportBar'
import { getMyProfile } from '@/app/(app)/cockpit/actions'
import { hasPermission } from '@/utils/permissions'
import { createClient } from '@/utils/supabase/server'
import { getBiReportBySlug } from '@/lib/relatorios/catalog'
import { BI_PAGE_SIZE_DEFAULT, runBiReport } from '@/lib/relatorios/queries'
import { formatBiReportCell, formatBiReportResumo } from '@/lib/relatorios/format'

export async function generateMetadata(props: { params: Promise<{ slug: string }> }) {
  const { slug } = await props.params
  const def = getBiReportBySlug(slug)
  return { title: def ? `${def.title} | Relatórios` : 'Relatório | HuginFlow' }
}

function buildPageHref(slug: string, values: Record<string, string>, offset: number): string {
  ...
  return `/cockpit/relatorios/${slug}${qs ? `?${qs}` : ''}`
}

export default async function RelatorioSlugPage(props: {
  ...
  if (!empresaId) return <p className="text-gray-400">Empresa não identificada.</p>
  ...
  const pageSize = BI_PAGE_SIZE_DEFAULT
  const offset = Math.max(Number(get('offset') || '0') || 0, 0)

  const filters = {
    data_inicio: get('data_inicio') || undefined,
    data_fim: get('data_fim') || undefined,
    departamento_id: get('departamento_id') || undefined,
    canal_id: get('canal_id') || undefined,
    pipeline_id: get('pipeline_id') || undefined,
    responsavel_id: get('responsavel_id') || undefined,
    min_cards: get('min_cards') ? Number(get('min_cards')) : undefined,
    limit: pageSize,
    offset,
  }

  const supabase = await createClient()
  const [deptos, canais, pipelines, users, report] = await Promise.all([
    supabase
      .from('departamentos')
      .select('id, nome')
    ...
    supabase
      ...
      .order('nome_completo')
      .limit(300),
    runBiReport(slug, empresaId, filters, supabase),
  ])
  ...
  const displayRows = report.rows.map((row) => {
    ...
    for (const c of def.columns) {
      out[c.key] = formatBiReportCell(row[c.key], c.format)
    }
    return out
  })
  ...
  return (
    <div data-testid={`report-page-${slug}`} className="space-y-6 pb-20">
      ...
      <BiReportFiltersForm
        ...
        values={values}
        departamentos={(deptos.data || []).map((d) => ({ id: d.id, label: d.nome }))}
        canais={(canais.data || []).map((c) => ({
          id: c.id,
          label: `${c.nome}${c.tipo ? ` · ${c.tipo}` : ''}`,
        }))}
        pipelines={(pipelines.data || []).map((p) => ({ id: p.id, label: p.nome }))}
        responsaveis={(users.data || []).map((u) => ({
          id: u.id,
          label: u.nome_completo || u.email || u.id,
        }))}
      />
        ...
        <div className="flex flex-wrap gap-3 text-xs text-gray-300">
          {Object.entries(report.resumo).map(([k, v]) => (
            ...
            >
              ...
              <span className="font-semibold text-white">{formatBiReportResumo(k, v)}</span>
            </span>
          ))}
        </div>
      )}
