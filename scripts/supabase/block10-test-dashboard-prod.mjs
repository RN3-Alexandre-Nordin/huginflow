/**
 * Bloco 10 — dashboard do gestor em PRODUÇÃO (10.1–10.3).
 * Usa tenant de scripts/supabase/out/prod-test-tenant.json
 *
 * Uso: node scripts/supabase/block10-test-dashboard-prod.mjs
 */
...
async function main() {
  ...
  const { data: profile } = await sb
    .from('usuarios')
    .select('id, empresa_id, role_global')
      ...
      JSON.stringify(
        {
          ...
          metrics,
          chartLengths: {
            dia: chartDia.pontos.length,
            semana: chartSemana.pontos.length,
            mes: chartMes.pontos.length,
          },
        },
        null,
        2,
